...
{
	return (((first&0x0f) + (second&0x0f)) & 0x10) 
		? Cpu::Flag_H : static_cast<Cpu::Flags>(0);
}
...
{
	return (((first&0xf00) + (second&0xf00)) & 0x1000) 
		? Cpu::Flag_H : static_cast<Cpu::Flags>(0);
}
...
constexpr Cpu::Flags fcheck_c_bit15(const uint32_t result) 
{
	return (result & 0xffff0000) 
		? Cpu::Flag_C : static_cast<Cpu::Flags>(0);
}
...
{
	return (((first&0xf) - (second&0xf)) < 0) 
		? Cpu::Flag_H : static_cast<Cpu::Flags>(0);
}



inline Cpu::Flags get_flags(const Cpu& cpu, const Cpu::Flags flags)
{
	return static_cast<Cpu::Flags>(cpu.f & flags);
}


inline void set_flags(const Cpu::Flags flags, Cpu* const cpu)
{
	cpu->f |= flags;
}
