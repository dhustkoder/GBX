...
inline void set(const uint8_t bit, uint8_t* const reg) {
	*reg = SetBit(bit, *reg);
}

inline void res(const uint8_t bit, uint8_t* const reg) {
	*reg = ResBit(bit, *reg);
}
...
{
	// SET 6, A
	set(6, &gb->cpu.af.ind.a);
}

void set_F8(Gameboy* const gb)
{ 
	// SET 7, B
	set(7, &gb->cpu.bc.ind.b);
}
